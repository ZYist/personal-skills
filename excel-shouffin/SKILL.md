...
| # | 不要做什么 | 为什么 | 正确做法 |
|---|-----------|--------|---------|
| 1 | 不要手动拼 xlsx 二进制 | 无法保证格式和样式正确 | 用 `scripts/excel-writer` 从 JSON 生成 |
| 2 | 不要支持 `.xls` 格式 | 旧版二进制格式完全不同 | 提示用户先另存为 `.xlsx` |
| 3 | 不要跳过 JSON 结构确认直接输出 | 数据可能错位或丢失 | 🛑 先展示 JSON 摘要，用户确认后再生成 |
